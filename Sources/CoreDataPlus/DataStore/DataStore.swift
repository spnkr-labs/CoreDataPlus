...
public class Persist {
    ...
    public class func setup(_ mode: SetupMode = .automatic) {
        switch mode {
        case .automatic:
            let p = Persist.enable()
            CoreDataPlus.setup(viewContext: p.viewContext, backgroundContext: p.backgroundContext, logHandler: { _ in })
            break
        case .with(let viewContext, let backgroundContext):
            
            break
        case .withModelName(let name):
            
            break
        case .withPersistentContainer(let container):
    ...
    public class func enable(container: NSPersistentContainer) -> Persist {
        let persist = Persist.shared
        guard persist.didEnable == false else { return persist }
        
        persist.overriddenPersistentContainer = container
        
        CoreDataPlusLogger.shared.log("Enabled persistence a passed NSPersistentContainer")
        persist.didEnable = true
        
        return persist
    }
    
    public class func enable(modelName: String) -> Persist {
        let persist = Persist.shared
        guard persist.didEnable == false else { return persist }
        
        persist.modelName = modelName
        
        CoreDataPlusLogger.shared.log("Enabled persistence for data model '\(persist.modelName)'")
        persist.didEnable = true
        
        return persist
    }
    
    public class func enable() -> Persist {
        let persist = Persist.shared
        guard persist.didEnable == false else { return persist }
        
        if let autoModelName = persist.guessModelName() {
            persist.modelName = autoModelName
        }
        
        CoreDataPlusLogger.shared.log("Enabled persistence for data model '\(persist.modelName)' (auto-detected)")
        persist.didEnable = true
